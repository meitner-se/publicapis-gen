...
// TestCompareJSONSemantics tests the semantic JSON comparison functionality
func TestCompareJSONSemantics(t *testing.T) {
	// Test with identical JSON content
	t.Run("identical JSON content returns true", func(t *testing.T) {
		jsonData1 := []byte(`{"name": "test", "value": 123}`)
		jsonData2 := []byte(`{"name": "test", "value": 123}`)

		isEqual, err := compareJSONSemantics(jsonData1, jsonData2)

		assert.Nil(t, err, "Expected no error when comparing identical JSON")
		assert.True(t, isEqual, "Expected identical JSON to be considered equal")
	})

	// Test with different property order
	t.Run("different property order returns true", func(t *testing.T) {
		jsonData1 := []byte(`{"name": "test", "value": 123, "active": true}`)
		jsonData2 := []byte(`{"active": true, "value": 123, "name": "test"}`)

		isEqual, err := compareJSONSemantics(jsonData1, jsonData2)

		assert.Nil(t, err, "Expected no error when comparing reordered JSON")
		assert.True(t, isEqual, "Expected JSON with different property order to be considered equal")
	})

	// Test with different whitespace/formatting
	t.Run("different whitespace returns true", func(t *testing.T) {
		jsonData1 := []byte(`{"name":"test","value":123}`)
		jsonData2 := []byte(`{
  "name": "test",
  "value": 123
		...
		assert.Nil(t, err, "Expected no error when comparing differently formatted JSON")
		assert.True(t, isEqual, "Expected JSON with different formatting to be considered equal")
	})

	// Test with different content
	t.Run("different content returns false", func(t *testing.T) {
		jsonData1 := []byte(`{"name": "test", "value": 123}`)
		jsonData2 := []byte(`{"name": "test", "value": 456}`)

		isEqual, err := compareJSONSemantics(jsonData1, jsonData2)

		assert.Nil(t, err, "Expected no error when comparing different JSON")
		assert.False(t, isEqual, "Expected different JSON content to be considered not equal")
	})

	// Test with invalid JSON
	t.Run("invalid generated JSON returns error", func(t *testing.T) {
		jsonData1 := []byte(`{invalid json}`)
		jsonData2 := []byte(`{"name": "test", "value": 123}`)

		isEqual, err := compareJSONSemantics(jsonData1, jsonData2)

		assert.Error(t, err, "Expected error when parsing invalid generated JSON")
		assert.False(t, isEqual, "Expected comparison to fail with invalid JSON")
		assert.Contains(t, err.Error(), "failed to parse generated JSON")
	})

	t.Run("invalid disk JSON returns error", func(t *testing.T) {
		jsonData1 := []byte(`{"name": "test", "value": 123}`)
		jsonData2 := []byte(`{invalid json}`)

		isEqual, err := compareJSONSemantics(jsonData1, jsonData2)

		assert.Error(t, err, "Expected error when parsing invalid disk JSON")
		assert.False(t, isEqual, "Expected comparison to fail with invalid JSON")
		assert.Contains(t, err.Error(), "failed to parse disk JSON")
	})
}

// TestCompareYAMLSemantics tests the semantic YAML comparison functionality
func TestCompareYAMLSemantics(t *testing.T) {
	// Test with identical YAML content
	t.Run("identical YAML content returns true", func(t *testing.T) {
		yamlData1 := []byte(`name: test
value: 123`)
		yamlData2 := []byte(`name: test
value: 123`)

		isEqual, err := compareYAMLSemantics(yamlData1, yamlData2)

		assert.Nil(t, err, "Expected no error when comparing identical YAML")
		assert.True(t, isEqual, "Expected identical YAML to be considered equal")
	})
...
value: 123
active: true`)
		yamlData2 := []byte(`active: true
value: 123
name: test`)

		isEqual, err := compareYAMLSemantics(yamlData1, yamlData2)

		assert.Nil(t, err, "Expected no error when comparing reordered YAML")
		assert.True(t, isEqual, "Expected YAML with different property order to be considered equal")
	})
...
value:     123`)
		...
		assert.Nil(t, err, "Expected no error when comparing differently formatted YAML")
		assert.True(t, isEqual, "Expected YAML with different formatting to be considered equal")
	})
...
value: 456`)
		...
		assert.Nil(t, err, "Expected no error when comparing different YAML")
		assert.False(t, isEqual, "Expected different YAML content to be considered not equal")
	})

	// Test with invalid YAML
	t.Run("invalid generated YAML returns error", func(t *testing.T) {
		yamlData1 := []byte(`name: test
...
value: 123`)
		...
		assert.Error(t, err, "Expected error when parsing invalid generated YAML")
		assert.False(t, isEqual, "Expected comparison to fail with invalid YAML")
		assert.Contains(t, err.Error(), "failed to parse generated YAML")
	})
}

// TestCompareWithDiskFileJSON tests the JSON file comparison functionality
func TestCompareWithDiskFileJSON(t *testing.T) {
	// Create temporary directory for test files
	tempDir := t.TempDir()

	// Test with matching content
	t.Run("matching JSON files return no difference", func(t *testing.T) {
		// Create test file
		testFilePath := tempDir + "/test.json"
		originalJSON := []byte(`{"name": "test", "value": 123}`)
		err := os.WriteFile(testFilePath, originalJSON, 0644)
		require.Nil(t, err, "Failed to create test file")

		// Test with reordered JSON
		generatedJSON := []byte(`{"value": 123, "name": "test"}`)

		diff, err := compareWithDiskFileJSON(testFilePath, generatedJSON)

		assert.Nil(t, err, "Expected no error when comparing matching JSON files")
		assert.Empty(t, diff, "Expected no differences for semantically identical JSON")
	})

	// Test with different content
	t.Run("different JSON files return difference", func(t *testing.T) {
		// Create test file
		testFilePath := tempDir + "/test_different.json"
		originalJSON := []byte(`{"name": "test", "value": 123}`)
		err := os.WriteFile(testFilePath, originalJSON, 0644)
		require.Nil(t, err, "Failed to create test file")

		// Test with different content
		generatedJSON := []byte(`{"name": "test", "value": 456}`)

		diff, err := compareWithDiskFileJSON(testFilePath, generatedJSON)

		assert.Nil(t, err, "Expected no error when comparing different JSON files")
		assert.Equal(t, "content differs", diff, "Expected content differs message for different JSON")
	})

	// Test with non-existent file
	t.Run("non-existent file returns file not found", func(t *testing.T) {
		nonExistentPath := tempDir + "/nonexistent.json"
		generatedJSON := []byte(`{"name": "test", "value": 123}`)

		diff, err := compareWithDiskFileJSON(nonExistentPath, generatedJSON)

		assert.Nil(t, err, "Expected no error when file doesn't exist")
		assert.Equal(t, "file does not exist on disk", diff, "Expected file not found message")
	})
}

// TestDiffCommandConsistency tests that the diff command produces consistent results
func TestDiffCommandConsistency(t *testing.T) {
	// Create temporary directory for test files
	tempDir := t.TempDir()
	ctx := context.Background()
	...
	t.Run("multiple generations produce consistent diff results", func(t *testing.T) {
		// Generate OpenAPI JSON file
		testFilePath := tempDir + "/test-openapi.json"
		generatedData, err := generateOpenAPIBytes(ctx, testService)
		require.Nil(t, err, "Failed to generate OpenAPI bytes")

		err = os.WriteFile(testFilePath, generatedData, 0644)
		require.Nil(t, err, "Failed to write test file")

		// Test diff multiple times
		for i := 0; i < 5; i++ {
			diff, err := checkOpenAPIJSONDifference(ctx, testService, testFilePath)
			assert.Nil(t, err, "Expected no error on diff check iteration %d", i+1)
			assert.Empty(t, diff, "Expected no differences on iteration %d", i+1)
		}
	})

	t.Run("property order differences are ignored", func(t *testing.T) {
		// Generate OpenAPI JSON
		generatedData, err := generateOpenAPIBytes(ctx, testService)
		require.Nil(t, err, "Failed to generate OpenAPI bytes")

		// Parse and rewrite with different property order
		var jsonData interface{}
		err = json.Unmarshal(generatedData, &jsonData)
		require.Nil(t, err, "Failed to parse generated JSON")

		reorderedData, err := json.MarshalIndent(jsonData, "", "  ")
		require.Nil(t, err, "Failed to marshal reordered JSON")

		// Write reordered file
		testFilePath := tempDir + "/test-reordered.json"
		err = os.WriteFile(testFilePath, reorderedData, 0644)
		require.Nil(t, err, "Failed to write reordered test file")

		// Test diff with reordered file
		diff, err := checkOpenAPIJSONDifference(ctx, testService, testFilePath)
		assert.Nil(t, err, "Expected no error when comparing reordered JSON")
		assert.Empty(t, diff, "Expected no differences for reordered JSON properties")
	})
}
