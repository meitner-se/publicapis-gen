...
// GenerateOpenAPI generates an OpenAPI 3.1 document from a specification.Service
// and writes it to the provided buffer in JSON format.
func GenerateOpenAPI(buf *bytes.Buffer, service *specification.Service) error {
	...
	gen.Title = service.Name + apiTitleSuffix
	gen.Description = defaultAPIDescription

	// Generate OpenAPI document
	document, err := gen.generateFromService(service)
	if err != nil {
		return fmt.Errorf("failed to generate OpenAPI document: %w", err)
	}

	// Convert to JSON
	jsonBytes, err := gen.toJSON(document)
	if err != nil {
