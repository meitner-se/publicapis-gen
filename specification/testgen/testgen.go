...
func generateServerSetup(buf *bytes.Buffer, serviceName string, service *specification.Service, currentResource specification.Resource, currentEndpoint specification.Endpoint, apiPackageName string) error {
	...
	buf.WriteString("\t\t\t\t\treturn \"test-request-id\"\n")
	buf.WriteString("\t\t\t\t},\n")
	...
	buf.WriteString("\t\t\t\t},\n")
	buf.WriteString(fmt.Sprintf("\t\t\t\tErrorHook: func(ctx context.Context, requestContext %s.RequestContext, err error) (int, *%s.Error) {\n", apiPackageName, apiPackageName))
	buf.WriteString(fmt.Sprintf("\t\t\t\t\treturn http.StatusInternalServerError, &%s.Error{\n", apiPackageName))
...
func generateServeWithResponseTest(buf *bytes.Buffer, apiPackageName string) error {
	...
	buf.WriteString("\t\t},\n")
	buf.WriteString("\t\tErrorHook: func(ctx context.Context, requestContext " + apiPackageName + ".RequestContext, err error) (int, *" + apiPackageName + ".Error) {\n")
	buf.WriteString("\t\t\treturn http.StatusInternalServerError, &" + apiPackageName + ".Error{\n")
...
func generateHandleRequestTest(buf *bytes.Buffer, apiPackageName string) error {
	...
	buf.WriteString("\t\trequest, err := " + apiPackageName + ".HandleRequest[any, struct{}, struct{}, struct{}](c, RequestContext{RequestID: \"test-123\"}, server)\n")
	buf.WriteString("\t\tassert.Nil(t, err, \"Expected no error from handleRequest\")\n")
	...
	buf.WriteString("\t\t_, err := " + apiPackageName + ".HandleRequest[any, struct{}, struct{}, struct{}](c, RequestContext{RequestID: \"test-456\"}, server)\n")
	buf.WriteString("\t\tassert.NotNil(t, err, \"Expected error when session function fails\")\n")
	buf.WriteString("\t\t// Type assert to API Error\n")
	...
	buf.WriteString("\t\t_, err := " + apiPackageName + ".HandleRequest[any, struct{}, struct{}, TestBodyParams](c, RequestContext{RequestID: \"test-789\"}, server)\n")
	buf.WriteString("\t\tassert.NotNil(t, err, \"Expected error when JSON is invalid\")\n")
	buf.WriteString("\t\t// Type assert to API Error\n")
	buf.WriteString("\t\tapiError, ok := err.(*" + apiPackageName + ".Error)\n")
	buf.WriteString("\t\tassert.True(t, ok, \"Error should be of type *Error\")\n")
	...
	buf.WriteString("\t\t_, err = " + apiPackageName + ".HandleRequest[any, struct{}, struct{}, struct{}](c, RequestContext{RequestID: \"test-prehook\"}, server)\n")
	buf.WriteString("\t\tassert.Nil(t, err, \"Expected no error from handleRequest\")\n")
	...
	buf.WriteString("\t\t\t},\n")
	buf.WriteString("\t\t\tErrorHook: func(ctx context.Context, requestContext " + apiPackageName + ".RequestContext, err error) (int, *" + apiPackageName + ".Error) {\n")
	buf.WriteString("\t\t\t\treturn http.StatusForbidden, &" + apiPackageName + ".Error{\n")
	...
	buf.WriteString("\t\t_, err = " + apiPackageName + ".HandleRequest[any, struct{}, struct{}, struct{}](c, RequestContext{RequestID: \"test-prehook-error\"}, server)\n")
	buf.WriteString("\t\tassert.NotNil(t, err, \"Expected error when pre-hook fails\")\n")
	buf.WriteString("\t\tassert.Equal(t, \"pre-hook validation failed\", err.Error(), \"Error message should contain pre-hook error\")\n")
	buf.WriteString("\t})\n\n")
...
func generateInternalUtilityTests(buf *bytes.Buffer, service *specification.Service) error {
	...
	buf.WriteString("\t\t// Test handleRequest\n")
	buf.WriteString("\t\trequest, err := handleRequest[any, struct{}, struct{}, struct{}](c, RequestContext{RequestID: \"test-123\"}, server)\n")
	buf.WriteString("\t\tassert.Nil(t, err, \"Expected no error from handleRequest\")\n")
	buf.WriteString("\t\tassert.Equal(t, \"test-session\", request.Session, \"Session should be set\")\n")
	buf.WriteString("\t\tassert.Equal(t, \"test-123\", request.Context().RequestID, \"RequestID should be set\")\n")
	buf.WriteString("\t})\n\n")
	...
	buf.WriteString("\t\t// Test handleRequest with session error\n")
	buf.WriteString("\t\t_, err = handleRequest[any, struct{}, struct{}, struct{}](c, RequestContext{RequestID: \"test-456\"}, server)\n")
	buf.WriteString("\t\tassert.NotNil(t, err, \"Expected error when session function fails\")\n")
	buf.WriteString("\t\tassert.Equal(t, \"authentication failed\", err.Error(), \"Error message should contain session error\")\n")
	buf.WriteString("\t})\n\n")
	...
	buf.WriteString("\t\t// Test handleRequest with invalid JSON\n")
	buf.WriteString("\t\t_, err = handleRequest[any, struct{}, struct{}, TestBodyParams](c, RequestContext{RequestID: \"test-789\"}, server)\n")
	buf.WriteString("\t\tassert.NotNil(t, err, \"Expected error when JSON is invalid\")\n")
	buf.WriteString("\t\tassert.Equal(t, \"cannot decode json body params: invalid character 'i' looking for beginning of value\", err.Error(), \"Error message should mention JSON decoding\")\n")
	buf.WriteString("\t})\n\n")
	...
	buf.WriteString("\t\t// Test handleRequest with pre-hook\n")
	buf.WriteString("\t\t_, err = handleRequest[any, struct{}, struct{}, struct{}](c, RequestContext{RequestID: \"test-prehook\"}, server)\n")
	buf.WriteString("\t\tassert.Nil(t, err, \"Expected no error from handleRequest\")\n")
	buf.WriteString("\t\tassert.True(t, hookExecuted, \"Pre-hook should have been executed\")\n")
	buf.WriteString("\t\tassert.Equal(t, \"test-prehook\", capturedContext.RequestID, \"Pre-hook should receive correct RequestID\")\n")
	...
	buf.WriteString("\t\t\t},\n")
	buf.WriteString("\t\t\tErrorHook: func(ctx context.Context, requestContext RequestContext, err error) (int, *Error) {\n")
	buf.WriteString("\t\t\t\treturn http.StatusForbidden, &Error{\n")
	...
	buf.WriteString("\t\t// Test handleRequest with pre-hook error\n")
	buf.WriteString("\t\t_, err = handleRequest[any, struct{}, struct{}, struct{}](c, RequestContext{RequestID: \"test-prehook-error\"}, server)\n")
	buf.WriteString("\t\tassert.NotNil(t, err, \"Expected error when pre-hook fails\")\n")
	buf.WriteString("\t\tassert.Equal(t, \"pre-hook validation failed\", err.Error(), \"Error message should contain pre-hook error\")\n")
	buf.WriteString("\t})\n")
	buf.WriteString("}\n\n")
...
func generateInternalSessionHookTests(buf *bytes.Buffer) error {
	...
	buf.WriteString("\t\t\t},\n")
	buf.WriteString("\t\t\tGetSessionFunc: func(ctx context.Context, headers http.Header) (string, error) {\n")
	buf.WriteString("\t\t\t\treturn \"\", &Error{Code: ErrorCodeUnauthorized, Message: types.NewString(\"authentication failed\")}\n")
	buf.WriteString("\t\t\t},\n")
	buf.WriteString("\t\t\tErrorHook: func(ctx context.Context, requestContext RequestContext, err error) (int, *Error) {\n")
	buf.WriteString("\t\t\t\tapiError, ok := err.(*Error)\n")
	buf.WriteString("\t\t\t\tif ok {\n")
	buf.WriteString("\t\t\t\t\treturn apiError.HTTPStatusCode(), apiError\n")
	buf.WriteString("\t\t\t\t}\n")
	buf.WriteString("\t\t\t\treturn http.StatusInternalServerError, &Error{\n")
	buf.WriteString("\t\t\t\t\tCode:      ErrorCodeInternal,\n")
	buf.WriteString("\t\t\t\t\tMessage:   types.NewString(err.Error()),\n")
